...
### 9. TM-UX-001 — Seamless capture + recommendation — 🚧 IN PROGRESS
...
- **Phase A (this PR) — Recommendation:** every query response carries
  `related_entities` — 1-hop graph expansion from the top-k direct hits,
  excluding entities already in the primary result set, scored by
  `graph_proximity × vector_sim_to_query`. Surfaced via CLI
  (`tracemind query "…"` prints a `Related:` block) and MCP
  (`memory_query` response payload grows a `related_entities` field).
  Claude Code and any MCP client get "you might also want…" for free.
