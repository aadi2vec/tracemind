...
pub struct ConsolidationReport {
    ...
    /// LM-5d — Map-of-Content entities upserted this cycle (new or
    /// existing). Tracks how many auto-MOCs are live in the graph.
    pub mocs_upserted: usize,
    /// LM-5d — new `indexes` backlinks added to MOCs this cycle.
    pub moc_backlinks_added: usize,
}
...
impl<'a> Consolidator<'a> {
    ...
    pub fn consolidate(&self) -> ConsolidationReport {
        ...
        // Phase 3 — LM-5d: refresh auto-generated Maps of Content.
        // Today's heuristic groups entities by shared predicate-target
        // signature (e.g. all entities with `uses Rust`). When CLU-6
        // backfills HDBSCAN cluster labels, swap in
        // `generate_moc_for_cluster` at this call site — the call
        // contract is identical.
        self.refresh_mocs(&entities, &mut report);

        report
    }

    /// LM-5d — regenerate the auto-MOC layer. Builds heuristic groups
    /// from the current triple set, then upserts a MOC entity per
    /// group with backlinks (`indexes` predicate) to its members.
    fn refresh_mocs(
        &self,
        entities: &[tm_types::Entity],
        report: &mut ConsolidationReport,
    ) {
        let triples = match self.graph.list_all_triples() {
            Ok(t) => t,
            Err(_) => return,
        };
        if triples.is_empty() {
        ...
        // Require 3+ members so MOCs stay meaningful — a duo isn't
        // a "map of content".
        let groups = tm_graph::heuristic_moc_groups(&triples, &names, 3);
        for (label, members) in groups {
            // Cap members to top 200 to keep MOC backlinks manageable.
            let members: Vec<Uuid> = members.into_iter().take(200).collect();
            match tm_graph::upsert_moc(
                self.graph,
                &label,
                "auto-generated by Consolidator",
                &members,
            ) {
                Ok(res) => {
                    report.mocs_upserted += 1;
                    report.moc_backlinks_added += res.new_backlinks;
                }
                Err(_) => continue,
            }
        }
