...
impl RetrievalEngine {
    ...
    /// Returns true if scoring was applied, false if skipped (no cached tokens or no reranker).
    fn apply_colbert_maxsim(&self, ws: &mut QueryWorkspace, _query_text: &str) -> bool {
        ...
        // Encode query tokens — requires the colbert feature for encode_query
        #[cfg(feature = "colbert")]
        {
            ...
            if let Some(query_tokens) = query_tokens_opt {
                ...
                for (id, orig_score) in &ws.candidates {
                    if let Some((flat_embs, token_count, dim)) = cached_tokens.get(id) {
                        // Reconstruct per-token embeddings from flat storage
                        ...
                        // Blend: 0.6 * maxsim + 0.4 * original vector score
