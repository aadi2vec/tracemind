...
//! ```bash
//! tm-bench-memory \
//!   --dataset crates/tm-bench-memory/fixtures/persistence-mini.json \
//!   --runner tracemind \
//!   --output crates/tm-bench-memory/baselines/w3-mini.json
//! ```
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    let dataset = PersistenceDataset::load_from_path(&args.dataset)?;
    tracing::info!(pairs = dataset.pairs.len(), "loaded persistence dataset");
    ...
    let report = match args.runner {
        ...
        RunnerKind::Tracemind => {
            ...
            run(tm_bench_memory::TraceMindPersistenceRunner::new(cfg), &dataset).await?
        }
    };
