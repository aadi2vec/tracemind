...
- [ ] **UI-6 Demo fixture restore** — `tracemind demo restore` already exists (D-2); verify it still works against the C-0 schema (contexts + signals tables). If anything drifted, fix the seeded UUIDs.
- [ ] **UI-7 End-to-end smoke + recording** — run the full Tauri app: switch context via header dropdown, run scoped query, file 👍 on a good row + "Wrong context" on a bad row, re-run query, watch ranking shift. Capture as 60-90s screen recording. Add to `docs/DEMO_SCRIPT.md` as the new shot 2.
