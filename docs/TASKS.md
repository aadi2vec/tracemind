...
- [ ] **F-1 Positive signal CLI + storage** — new `positive_signals(query_id, result_id, weight, kind, ts)` table (mirror of `negative_signals`). `tracemind helpful <query_id> [<result_id>] [--weight w] [--kind k]` + MCP `memory_feedback {kind: "helpful"}`. The matching addition to `finalize_pending_reward` so the bandit *also* learns from positives, not just negatives. ~1 day. Unblocks everything else.
