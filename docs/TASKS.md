...
- [x] **CLU-1 `tm-cluster` crate scaffold + HDBSCAN crate evaluation** — new workspace member. Spike: evaluate the [`hdbscan`](https://crates.io/crates/hdbscan) Rust crate vs. Python sklearn HDBSCAN via PyO3 sidecar on 5k synthetic capture events. Compare cluster persistence, outlier rate, latency. Pick winner. Public API: `Clusterer::open(db_path)`, `Clusterer::recluster() -> ClusterStats`, `Clusterer::assign(event_id, embedding) -> Assignment { cluster_id, membership_prob, is_outlier }`, `Clusterer::recent_centroids(window: Duration) -> Vec<(ClusterId, f32, MembershipProb)>`, `Clusterer::outliers(window) -> Vec<EventId>`. Two SQLite tables: `clusters(id, label_text, centroid_blob, n_members, persistence, is_outlier_bucket, updated_at)`, `event_clusters(event_id, cluster_id, membership_prob, is_outlier)`. *(2026-05-16: shipped — `tm-cluster` workspace member with `hdbscan_impl.rs` (Rust crate path picked over the sidecar), `assign.rs` for CLU-3, `store.rs` for the two tables, public API as specified.)*
- [x] **CLU-2 HDBSCAN batch re-clusterer** — full HDBSCAN pass on a background thread every 100 events OR every 10 min, whichever first. Uses cosine distance on 384-d BGE embeddings. Persistence threshold defaults to `min_cluster_size = 5`, `min_samples = 3` — tune on real DP data. **No `k` parameter.** *(2026-05-16: `Clusterer::recluster<I>` shipped; wired into Tauri `cmd_consolidate` via the new `GraphStore::all_signals_with_embeddings(limit)` source so a manual consolidate triggers a full pass. The "every 100 events / every 10 min" background scheduler is deferred — manual + consolidate-time coverage is enough for the alpha; add the timer in P4c if a partner notices stale clusters.)*
- [x] **CLU-3 Nearest-centroid assignment between re-clusters** — new events arriving between full re-clusters get assigned to the nearest existing cluster centroid (cosine sim) if within `2σ` of the cluster's intra-distance; otherwise tagged outlier (`cluster_id = -1`). Outliers contribute their own embedding to the L1 topic vector directly. *(2026-05-16: `tm_cluster::assign::assign_event` shipped, called from `IngestPipeline::ingest_fast`.)*
- [x] **CLU-4 Capture pipeline hook** — `IngestPipeline.ingest_fast` calls `Clusterer::assign` after `VectorStore.embed` and persists the assignment. Adds ≤ 2ms to the ingest path (target). Full re-cluster is async and never blocks ingest. *(2026-05-16: shipped in `tm-ingest/src/pipeline.rs` — `assign` is invoked with the same `format!("{:016x}", hash64)` content-hash key the re-cluster path now uses, so subsequent re-clusters update rather than duplicate.)*
- [x] **CLU-5 `tm-graph` Louvain community detection** — ~200 LOC community pass over `kg_relations` (or pull `leiden-rs`). Annotates `kg_entities` with `community_id`. Runs nightly via `consolidate`. **Sibling to HDBSCAN clusters, not a replacement** — HDBSCAN clusters events (topical/temporal), Louvain clusters entities (relational). Both feed WME and the P5a Obsidian surface. *(2026-05-16: `tm_graph::community::recompute_communities` shipped (single-level Louvain over `kg_relations` weighted by confidence) + `GraphStore::recompute_communities` wrapper + wired into Tauri `cmd_consolidate`. `kg_entities.community_id` ALTERed in-place on first run.)*
- [x] **CLU-6 c-TF-IDF auto-label per cluster** — for each HDBSCAN cluster, run c-TF-IDF over the member memory texts → top-N terms → human-readable label ("rondo + veo3 + scouting"). ~50 LOC. Cached on `clusters.label_text`; regenerated on re-cluster. Feeds P5a MOCs + P5e Memory Garden. *(2026-05-12: shipped as `tm_graph::labeler` (`label_clusters`, `LabelerConfig`, smooth IDF + bigram boost). `GraphStore::cluster_labels(n)` wraps it; Memory Garden reads it. See LM-21 note.)*
- [x] **CLU-7 Outlier surfacing API** — `Clusterer::outliers(since: Duration) -> Vec<EventId>` returns recent outlier points. Feeds WME "Anticipate" verb (novel topics) + Tauri "Unsorted" tray (P5e). **Outliers are first-class signal, not noise.** *(2026-05-12: outlier API + `cmd_outliers_list` + triage shipped per LM-22.)*
- [ ] **CLU-8 Python-sidecar fallback path (conditional)** — if CLU-1 spike picks the sidecar, scaffold `tm-cluster::sklearn_sidecar` (subprocess or PyO3) that runs HDBSCAN nightly off the request path. Sync output to `clusters` + `event_clusters` tables. **Off the hot path** — Tier-0 retrieval keeps working without fresh clusters. *(N/A — CLU-1 picked the pure-Rust path, so this fallback never needed scaffolding. Leave open until/unless we hit a quality regression that forces the sklearn comparison.)*
...
- [x] **WME-1 L1 rolling topic vector** — `tm-reflect::WorkingMemoryEngine::topic_vector(window: Duration) -> Vec<f32>`. EMA of recent cluster centroids weighted by recency over a 5-min decaying window. Updates every 30s or on significant capture event. *(2026-05-16: shipped in `tm-reflect::working_memory`.)*
- [x] **WME-2 L2 proactive retrieval driver** — `WorkingMemoryEngine::candidates(topic_vec, active_entities) -> Vec<Candidate>`. Joins three primitives: vector ANN against topic vector, graph 1-hop on active entities, cluster siblings. Excludes memories already seen this session. *(2026-05-16: shipped.)*
- [x] **WME-3 L3 verb-first card synthesis** — score = `relevance × surprise × recency_of_decision × outcome_signal`. Six verbs: Resume / Recall / Compare / Caution / Connect / Anticipate. Each verb is a typed `CardKind` with its own scoring rule; commitment+contradiction become filtered outputs of Caution+Compare. *(2026-05-16: six verb kinds + scorer shipped.)*
- [x] **WME-4 Card queue + cooldowns** — `CardQueue` with per-(card_kind, target_id) cooldown, per-session displayed-set, hard cap ≤ 8 cards/hour. Producer/consumer split: WME never blocks query path. *(2026-05-16: shipped; cooldowns persisted via `wme_feedback`.)*
- [x] **WME-5 Brief + Next Actions wiring** — Tauri `BriefView` + `DashboardView` consume `cmd_wme_cards()` instead of the current flat commitment/contradiction list. Reactive panels become anticipatory panels. *(2026-05-16: shipped — new `cmd_wme_cards` / `cmd_wme_feedback` Tauri commands, `WmeCardsPanel.tsx` mounted in both `BriefView` and `Dashboard`.)*
- [x] **WME-6 Feedback signals** — `memory_feedback {kind: "useful_now" | "not_useful_now" | "not_now_remind_later" | "dismiss_this_kind"}` MCP tool + Tauri buttons. Retrains card-score threshold per signal (per-user, per-card-kind). *(2026-05-16: `wme_feedback` table + per-kind decayed outcome aggregator shipped; Tauri 4-button row wired.)*
- [x] **WME-7 MCP tool `memory_brief`** — exposes the active WME card set to MCP hosts. Claude Code / Cursor can ask "what should I be thinking about?" without a query and get verb cards back. *(2026-05-16: `handle_memory_brief` now attaches `wme_cards` (default limit 12, opt-out with `wme_limit=0`).)*

### P4c — WME evaluation + guardrails

- [x] **WME-8 Useful-rate harness** — replay tool: feed a captured session log through WME, compute proactive-card useful-rate against ground-truth annotation. Target ≥ 30% by Q3-end, ≥ 45% by Q4 (PROJECT_2026.md §7 metric row). *(2026-05-16: shipped as the `tm-bench-wme` useful-rate gate (≥ 0.60 on the synthetic acceptance policy). Real-DP replay harness still TBD — current bench is synthetic only.)*
- [x] **WME-9 Anti-spam regression test** — synthetic 1-hour session with 1000 capture events; assert `cards_surfaced ≤ 8`. PERF-2 CI gate. *(2026-05-16: shipped in `tm-bench-wme`.)*
- [x] **WME-10 WME tick latency benchmark** — `cargo bench --bench wme_tick`; target ≤ 50ms p95. Wired into PERF-1 baseline. *(2026-05-16: shipped — `tm-bench-wme` per-tick p95 gate at 50ms over 1000 ticks. Add a `cargo bench` wrapper later if we want criterion-style historical traces; the gate is binary today.)*
...
- [x] **LM-3 Entity drawer rewrite** — replace current static entity view with: header (name + type + community) → backlinks panel (LM-1) → relations table → recent captures → cluster siblings (from `tm-cluster`). *(2026-05-12: `cmd_entity_drawer` returns composite `EntityDrawerView { header, backlinks, relations, tags }`. Cluster-sibling row appears once `tm-cluster` lands.)*
- [x] **LM-4 Live graph update on capture** — when CAP-* ingests a new triple touching the active entity, the open entity drawer refreshes without reload. Tauri event channel. *(2026-05-12: `entity-updated` events emitted from clipboard capture loop.)*
- [x] **LM-5a Transclusion / memory embeds** — one memory can reference-embed another inline; embedded memory renders as a styled blockquote with a link to the source. Auto-triggered when a memory is summarised by another. *(2026-05-12: `cmd_resolve_transclusion` parses `![[entity_id_or_name]]` spans.)*
- [x] **LM-5b Auto-tags from cluster labels + heuristic hashtags** — every memory carries `tags: Vec<String>` derived from (a) HDBSCAN cluster label (CLU-6), (b) Louvain community label, (c) any literal `#hashtag` the user happens to type. *(2026-05-12: `tm-ingest::tags` ships word-boundary hashtag extraction + entity-type fallback. Cluster-label tags wire in when CLU-6 lands.)*
...
- [x] **LM-5d Auto-generated MOCs (Maps of Content)** — for each persistent HDBSCAN cluster + each Louvain community, generate a `MapOfContent` memory: title (from c-TF-IDF label), description (top-3 representative memories), backlinks to all member memories. *(2026-05-12: `tm-graph::moc` ships `upsert_moc`, `generate_moc_for_cluster` (CLU-6 hook), and `heuristic_moc_groups` fallback that clusters by shared predicate-target signature today. Idempotent by name. `EntityType::MapOfContent` lives in `tm-types`.)*
- [x] **LM-5e Force-directed graph view (Q4 polish)** — Tauri Memory Garden full-graph mode. Anti-spam: never render > 500 nodes raw; collapse to cluster-summary view above that. *(2026-05-12: shipped early in `GraphView.tsx` — canvas-based Verlet integration with community-anchored gravity, per-community ring layout, type/community color modes, min-degree filter, drag-to-reposition, focus-on-community drill-down (`tm:focus-community`), and clickable labeled legend pills built from highest-degree entity names. Live data via `cmd_graph` with on-the-fly Louvain.)*
...
- [ ] **LM-6 SML candidate evaluation** — score REBEL (460M BART), GLiNER-Relation (~150MB), Qwen 2.5 0.5B-prompted, Phi-3-mini-4k on a 200-sentence eval set (precision, recall, per-sentence latency). Pick winner. Cheapest path: reuse Qwen (already auto-downloaded for Tier-1). Reference partner-recommended Jaya Gupta graph-extraction guide (link TBD when work item opens). *(Deferred 2026-05-12: requires GPU eval rig + 200-sentence labeled set. Blocked on benchmarks, not code. The LM-8 worker scaffold accepts a `Box<dyn EntityExtractor>` so any winner plugs in without API churn.)*
...
- [x] **LM-8 Async triple-extraction worker** — `tm-ingest::TripleWorker` runs the chosen SML off the ingest hot path on a tokio bounded channel. Heuristic NER stays as the synchronous fast path; SML enriches asynchronously. Persists new triples with confidence. *(2026-05-12: shipped on `std::sync::mpsc::sync_channel` for non-tokio integration; routes triples through `route_triple_by_confidence` so high-confidence rows enter `kg_relations` and low-confidence rows land in `pending_relations`. SML swap = pass a different `Box<dyn EntityExtractor>` to `spawn`.)*
...
- [x] **LM-11e Session-scoped splice (Q3, Tauri)** — Tauri thread sidebar shows active view; multi-select memories → "Use these for next query in this thread." Live-editable. *(2026-05-12: `cmd_thread_view_save / load / clear` persist `ThreadViewState` per-thread to `thread_views.json` next to `memory.db`. Canvas/whiteboard surface (LM-5f) reads from the same store.)*
...
- [x] **LM-15 Ontological typing (Q4 expensive path)** — SML pass classifies entities into domains (`fiction.location`, `real.location`, `historical.event`, `concept`, `person.real`, `person.fictional`, etc.). Bridges denied across disjoint domains. *(2026-05-12: `tm-types::OntologicalDomain` enum + `is_compatible_with` ships now; `tm-graph::ontology::classify` provides a heuristic classifier (fictional-name hints for Harry Potter / Tolkien / Star Wars / D&D); `DenyList::is_bridge_blocked_by_ontology` blocks real↔fictional cross-domain bridges. Swapping in an SML classifier is purely a `classify_ontology` re-implementation — call-site is stable.)*
...
- [x] **LM-19 Optional PII redaction pass (Q4)** — `--redact` flag runs `tm-governance` PII scrub before write. Off by default; on for "share with someone else" mode.

### P5e — Cluster-sort UI on `tm-cluster` (HDBSCAN-driven, revised 2026-05-12)

- [x] **LM-20 Memory Garden view (Q3)** — Tauri grid of all memories grouped by HDBSCAN `cluster_id`. Each cluster card: human-readable c-TF-IDF label (CLU-6) + count + 3 sample entities + persistence score. Click → drill into cluster. Outlier bucket (`cluster_id = -1`) renders as separate "Unsorted" tray. *(2026-05-12: `cmd_memory_garden` reads `captured_signals.cluster_id` via `GraphStore::cluster_buckets/cluster_samples`. c-TF-IDF labels appear once CLU-6 lands.)*
- [x] **LM-21 Auto-cluster labels** — supplied by c-TF-IDF (originally planned for CLU-6 inside `tm-cluster`). Memory Garden just reads them. Optional override: user can rename a cluster label, persisted to `clusters.label_text`. *(2026-05-12: shipped without waiting on the `tm-cluster` crate — c-TF-IDF labeler lives in `tm-graph::labeler` (`label_clusters` + `LabelerConfig`, smooth IDF + bigram boost). `GraphStore::cluster_labels(n)` wraps it; `cmd_memory_garden` calls it at `main.rs:3258` so every Memory Garden card with a real cluster id renders a topic phrase like "vector search / embeddings / retrieval" instead of "cluster 7". Outlier and ignore buckets keep the static "unsorted" / "ignored" labels. Same-day bonus: community labels for the Graph legend / Garden community overlay built from top-3-degree entity names (`cmd_graph.community_labels`, `CommunityRow.label`) so the Louvain side is searchable too.)*
- [x] **LM-22 Outlier "Unsorted" tray + triage** — outlier events (CLU-7) shown in a dedicated Tauri tray. User actions per outlier: "add to existing cluster X," "create new cluster," "ignore." Triage feeds back to `event_clusters` so the next HDBSCAN pass has a head start. **Outliers are signal, not noise.** *(2026-05-12: `cmd_outliers_list` + `cmd_outlier_triage` (`add_to | new_cluster | ignore` → -2). Per HDBSCAN convention: `cluster_id = NULL/-1` is unsorted, `-2` is user-marked "ignore".)*
- [x] **LM-23 Community-overlay toggle** — Memory Garden has an overlay mode that colors memories by Louvain `community_id` instead of HDBSCAN `cluster_id`. Lets the user see both organizational axes. *(2026-05-12: `cmd_community_overlay` ships. `GraphStore::community_buckets` gracefully synthesizes a single "unassigned" bucket when the `kg_entities.community_id` column isn't yet populated by CLU-* work — no schema migration needed.)*
- [ ] **LM-24 evoc / UMAP swap evaluation (Q4)** — if `evoc` (TutteInstitute) or a maintained Rust UMAP reaches production grade, A/B vs. native HDBSCAN on the Memory Garden quality (cluster-cohesion + user "this cluster makes sense" rating). Swap if A/B wins ≥ 2x. *(Deferred 2026-05-12: A/B experiment, not implementation. Re-evaluate once `evoc` Rust bindings stabilize.)*
