...
- [ ] **LM-21 Auto-cluster labels** — supplied by CLU-6 (c-TF-IDF in `tm-cluster`). Memory Garden just reads them. Optional override: user can rename a cluster label, persisted to `clusters.label_text`. *(Deferred 2026-05-12: blocked on `tm-cluster` crate, not yet introduced to the workspace. Memory Garden already renders `"cluster {id}"` as a stable label and will flip to c-TF-IDF labels transparently once CLU-6 backfills the column.)*
